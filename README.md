...
The pretrained `.pt` checkpoint files are stored in the repository via
**Git LFS**.

Important:

- Do **not** use GitHub's autogenerated **Source code (zip)** download.
- That ZIP contains only small Git LFS pointer files, not the real
  checkpoint blobs.
- Reviewers should obtain the artifact through a normal `git clone` and
  then materialize the checkpoints with `bash fetch_checkpoints.sh`.

Recommended checkout procedure:
...
If the repository was already cloned but the checkpoints are still
pointers, run:
...
1. Checks that the directory is a real Git checkout
2. Runs `git lfs pull` for the required `.pt` files
3. Verifies that the downloaded files are real checkpoints rather than
   Git LFS pointer stubs

Quick verification:

``` bash
file saved_models/alexnet_cifar10_target.pt
```
