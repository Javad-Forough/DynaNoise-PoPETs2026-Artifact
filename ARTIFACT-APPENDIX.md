...
The repository stores the pretrained `.pt` files through **Git LFS**.
GitHub's autogenerated source ZIP must not be used for Artifact
Evaluation because it contains Git LFS pointer stubs instead of the real
checkpoint blobs.
...
If the repository has already been cloned, but the checkpoint files are
still pointers, run:
...
To confirm that the checkpoints are real blobs, for example:
